	    ...
	    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
	        ...
	        if (id != null && id.length() > 0) {
	        	if (parserContext.getRegistry().containsBeanDefinition(id))  {
	        		RootBeanDefinition reDb = (RootBeanDefinition)parserContext.getRegistry().getBeanDefinition(id);
	        		if(!reDb.getBeanClass().equals(beanDefinition.getBeanClass())){
	        			throw new IllegalStateException("Illegal bean ,id does not match class!");
	        		}
	         	}
