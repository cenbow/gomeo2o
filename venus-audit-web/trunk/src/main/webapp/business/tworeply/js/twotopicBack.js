$(function(){
	if(window.util.query("type")==1){
		document.title = '未审核 - 二级回复';
	}else if(window.util.query("type")==2){
		document.title = '审核已通过 - 二级回复';
	}else if(window.util.query("type")==3){
		document.title = '审核未通过 - 二级回复';
	}
	...
	var twoTopicBack={
			id:$("#exaTwoTopicBack"),
			url: "../../twoTopicBack/findTwoTopicBack",
			...
			mData:[{"mData" : "id","sWidth" : "3%","class": "center sorting","bSearchable": false,"bStorable": false,render:function(data, type, row) {
	           ...
	           {"mData" : "firstReplyId","class": "center sorting"},
	       ...
	       externalEvents:[function(){
	   			//执行顶部事件
	   			window.util.bindType($(twoTopicBack.searchKey),window.validate.click,function(event){
	   				window.search=[{"name" : "topicBackContent","value" : encodeURI($("#topicBackContent").val())},{"name" : "type","value" : window.util.query("type")}];
	   				//再次加载数据
	   				window.mx.initData(twoTopicBack);
	   			});
	   	   },function(){
  			//批量审核通过
  			window.util.bindType($('#batchaudit'),window.validate.click,function(event){
	    		   ...
	    		   if(window.check.length>0){
	    			   window.util.post('../../twoTopicBack/egis',{check: JSON.stringify(window.params)},function(msg){
	    				   if(!msg.success){
	    					   alert(msg.message);
	    				   }else{
	    					  window.search=[{"name" : "topicBackContent","value" : encodeURI($("#topicBackContent").val())},{"name" : "type","value" : window.util.query("type")}];
   	   ...
   	   //审核通过
   	   window.util.bindType($(twoTopicBack.id).find('button.btn-success'),window.validate.click,function(event){
   		   window.params={
   		      check:$(this).data('id'),
   		      egis:true,
   		      singleton:true,
   		   ...
   		   if($(this).data('id')!=undefined){
   		    ...
   		    //审核不通过和批量审核不通过
   		 	...
   		 	window.util.bindType($(twoTopicBack.id).find('button.btn-danger'),window.validate.click,function(event){
	...
	//虚拟挂载搜索
	window.search=twoTopicBack.search;
	//外部事件的挂载
	window.externalEvents=twoTopicBack.externalEvents;
	//初始化
	window.mx.init(twoTopicBack);
})