$(function(){
	if(window.util.query("type")==1){
		document.title = '未审核 - 话题';
	}else if(window.util.query("type")==2){
		document.title = '审核已通过 - 话题';
	}else if(window.util.query("type")==3){
		document.title = '审核未通过 - 话题';
	}
	...
	var topic={
			id:$("#exaTopic"),
			url: "../../topic/findTopic",
			search:[{"name" : "topicName","value" : encodeURI($("#topicName").val())},{"name" : "type","value" : window.util.query("type")}],
			mData:[{"mData" : "id","sWidth" : "3%","class": "center sorting","bSearchable": false,"bStorable": false,render:function(data, type, row) {
	           ...
	           {"mData" : "topicPic","sWidth" : "15%","class": "center sorting", render:function (data, type, row) {
	        	   var img = "<div>";
	        	   if(data!=null&&data!=""){
	           ...
	           {"mData" : "content","class": "center sorting", render:function (data, type, row) {
	        	   /*if(data!=""&&data!=null){
	        		   var img = '';
	        		   img+='<div class="main" id="mainDiv">';
	        			 	img+=data;	
	        				   img+='<div class="intro">';
	        					   img+='<a class="key">展开</a>';
	        				img+='</div>';
	        			img+='</div>';
		        	   if(data.length>5){
		        		   return data.substr(0,5)+"...<a href='javaScript:;'>展开</a>";
		        	   }
	        		  return img;
	        	   }*/
	        	   return data;  
	           ...
	           {"sWidth" : "110px","mData" : "createOperator", "class": "center sorting"},
	       ...
	       externalEvents:[function(){
		   			...
		   			window.util.bindType($(topic.searchKey),window.validate.click,function(event){
		   				window.search=[{"name" : "topicName","value" : encodeURI($("#topicName").val())},{"name" : "type","value" : window.util.query("type")}];
		   				//再次加载数据
	   			...
	   			window.util.bindType($('#batchaudit'),window.validate.click,function(event){
		    		   window.params={
	 	    		   ...
	 	    		   if(window.check.length>0){
	 	    			   window.util.post('../../topic/egis',{check: JSON.stringify(window.params)},function(msg){
	 	    				   if(!msg.success){
	 	    					   alert(msg.message);
	 	    				   }else{
	 	    					  window.search=[{"name" : "topicName","value" : encodeURI($("#topicName").val())},{"name" : "type","value" : window.util.query("type")}];
