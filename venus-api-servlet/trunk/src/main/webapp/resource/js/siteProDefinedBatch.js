...
//弹出第三方属性自定义添加层
  function propertyDefined(){
	//判断是否选择分类
	var selectedSiteCate = $("#siteCategoryTab").val(); 
	if(selectedSiteCate!="qxz" && selectedSiteCate!="siteCategorty"){
	  	//一、数据赋值（站点、分类、商品编号）
	  	$("#site_name").val($("#site").combo("getText"));
	  	$("#site_cateId").val(selectedSiteCate.split(">")[0]);
	  	$("#site_cateName").val($("#siteCategoryTab").find("option:selected").text());
	  	var siteId = $("#site").combo("getValue"); 
	  	//二、弹出框展示
	...
	function accept(dgId) {
		...
		if ($dg.datagrid('getChanges').length) {
          ...
          if(!checkState){
        	 ...
        	 if(dgId==0){
	        	  //追加到主页面上（属性、属性值）
	        	  $.post("/siteCategory/spupdate",{'jsons':'['+jsonArray.join(",")+']','thirdCateId':$('#site_cateId').val(),'siteId':$("#site").combo("getValue")},function(data){
	        	  },'jsonp');
        	  }else{
        		//追加到主页面上（属性、属性值）
