$(function(){
	...
	var groupLog={
			id:$("#exaGroupLog"),
			url: "../../../log/groupLog",
			search:[{"name" : "userId","value" : encodeURI($("#userId").val())},{"name" : "dbtable","value" : "editGroupLog"}],
			mData:[
	         ...
	         function(){
	    	   //再次加载数据
    			window.util.bindType($(groupLog.searchKey),window.validate.click,function(event){
    				window.search=[{"name" : "userId","value" : encodeURI($("#userId").val())},{"name" : "dbtable","value" : "editGroupLog"}];
