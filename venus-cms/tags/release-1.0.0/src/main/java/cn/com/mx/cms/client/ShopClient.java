...
//			throw new JsonResponseException(result.getMessage());
	...
	public Map<String, Boolean> getShopPrivilege(RecommendDto dto) throws Exception {
		...
		if ("2".equals(vshopInfo.getVshopType())) {
			if (vshopInfo.getPopId() == null) {
				log.error("数据库数据问题，vshopId没有对应的popId,参数，vshopId:" + id);
				throw CmsException.VSHOP_INFO_FAILE;
			}
			// 红包(最高红包金额)---
			try {
				Response<CouponDefinition> findMostDiscount = couponReadService.findMostDiscount(Long
						.parseLong(vshopInfo.getPopId()));
			...
			// 店铺特惠(最高优惠额度)
			try {
				Response<ItemActivity> findMostDiscount2 = itemActivityReadService.findMostDiscount(Long
						.parseLong(vshopInfo.getPopId()));
