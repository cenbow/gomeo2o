...
public class RebateClient {
	 ...
	 *            用户角色ID
	 */
	public String getItemRebateAmount(int rebateType, long itemId, int itemPrice, int userRoleId) throws Exception {
		// 获取最大返利金额
		String maxRebatePrice = getShareMaxRebatePriceByItem(rebateType, itemId, itemPrice, userRoleId);
		
		// 如果没有获取最大返利金额，采用些方案
		if (Double.valueOf(maxRebatePrice) <= 0) {
			return getShareRebatePriceByItem(rebateType, itemId, itemPrice, userRoleId);
		}
	 ...
	 * @param userRoleId
	 */
	private String getShareMaxRebatePriceByItem(int rebateType, long itemId, int itemPrice, int userRoleId) {
		// 获取商品SKU最大价格
		...
		if (returnMap.size() > 0) {
			for (Entry<Long, BigDecimal> entry : returnMap.entrySet()) {
				...
				// 从计算中心取到商品返利百分比
				JSONObject json = JSONObject.fromObject(cacheService.hget(getRebate(), "Rebate_Calc", "Rebate_Calc_"
						+ entry.getKey() + ""));
				if (json != null && json.size() > 0) {
					...
					if (rebateType == RebateType.buyShare.getCode()) {// pmtShareRatio
						return df5.format(ChangePriceUtil.add(
								ChangePriceUtil.add(json.getDouble("popShareRatio"), json.getDouble("pfShareRatio")),
								json.getDouble("pmtShareRatio")) * (skuMaxPrice.doubleValue()));
					}
				}
				break;
			}
	...
	private Map<Long, BigDecimal> getMaxPriceByItem(long itemId) {
		...
		if (!res.isSuccess()) {
			...
			if (res.getResult() != null) {
				...
				DecimalFormat df5 = new DecimalFormat("0.00");
				df5.setRoundingMode(RoundingMode.HALF_UP);
				skuMaxPrice = new BigDecimal(df5.format(Double.valueOf(skuUpprice) /100));
	...
	/**
	 * 
	 * @Description: 获得分享返利金额
	 */
	private String getShareRebatePriceByItem(int rebateType, long itemId, int itemPrice, int userRoleId){
		int price = itemPrice;
		...
		if (json != null && json.size() > 0) {
			...
			if (rebateType == RebateType.buyShare.getCode()) {// pmtShareRatio
				return df5.format(ChangePriceUtil.add(
						ChangePriceUtil.add(json.getDouble("popShareRatio"), json.getDouble("pfShareRatio")),
						json.getDouble("pmtShareRatio"))
						* (Double.valueOf(ChangePriceUtil.formatDiv(Double.valueOf(price), 100d))));
				// 分销
			} else {
				if (userRoleId == 3) {// 达人 pmtCommonUserRatio
					return df5.format(ChangePriceUtil.add(
							ChangePriceUtil.add(json.getDouble("popDistributionRatio"),
									json.getDouble("commonUserRatio")), json.getDouble("pmtCommonUserRatio"))
							* (Double.valueOf(ChangePriceUtil.formatDiv(Double.valueOf(price), 100d))));
				} else if (userRoleId == 6 || userRoleId == 7) {// 普通员工/门店员工
																// pmtGomeUserRatio
					return df5.format(ChangePriceUtil.add(ChangePriceUtil.add(json.getDouble("popDistributionRatio"),
							json.getDouble("gomeUserRatio")), json.getDouble("pmtGomeUserRatio"))
							* (Double.valueOf(ChangePriceUtil.formatDiv(Double.valueOf(price), 100d))));
				} else if (userRoleId == 8) {// 促销员工(卖家) pmtMerchantUserRatio
					return df5.format(ChangePriceUtil.add(
							ChangePriceUtil.add(json.getDouble("popDistributionRatio"),
									json.getDouble("merchantUserRatio")), json.getDouble("pmtMerchantUserRatio"))
							* (Double.valueOf(ChangePriceUtil.formatDiv(Double.valueOf(price), 100d))));
				} else {
					return df5.format((json.getDouble("popDistributionRatio") + 0)
							* (Double.valueOf(ChangePriceUtil.formatDiv(Double.valueOf(price), 100d))));// 如果是XPOP
				}
			}
		} else {
			return "0";
