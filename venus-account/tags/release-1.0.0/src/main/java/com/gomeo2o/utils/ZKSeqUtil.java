...
public class ZKSeqUtil {
	...
	private static final String ZK_HOST = PropertiesUtil.getProProperties("dubboRegistry", null).replace("zookeeper://", "").replace("?backup=", ",");
	private static CuratorFramework zkclient;
	private ZKSeqUtil() {
		try {
			RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
			zkclient = CuratorFrameworkFactory.newClient(ZK_HOST, retryPolicy);
			zkclient.start();
		} catch (Exception e) {
